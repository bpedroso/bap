...
Toda equipe deve conseguir dizer o que o sistema faz, seu contexto e suas fronteiras. O conhecimento do domínio deve estar na BASE; se a equipe não sabe, o sistema se torna menos confiável em situações de falha.

---

### Observabilidade é alfabetização

Logs, métricas e traces não são “trabalho de SRE depois”. São o mínimo para qualquer papel enxergar o sistema. Sem isso, arquitetura vira opinião.

---

### Security by design

Segurança não entra no fim do projeto: nasce com o domínio. Identidade, dados sensíveis e quem pode ver ou alterar o quê são decisões de fundação. Quando a proteção é desenhada junto com o sistema, evita-se o ciclo eterno de “patch de segurança”.
...
| Função | Insight |
|--------|---------|
| **Desenvolvedor júnior** | Querer saber como uma requisição chega (cliente → rede → API → serviço → dados) já localiza o próprio trabalho no sistema, seja back ou front. Esse mapa mínimo reduz o “código no vácuo”. |
| **Designer** | Um pouco de arquitetura amplia ideias e possibilidades. Saber que existe latência, cache, estados de carregamento ou limites de API muda o que vale desenhar — e o que vale defender. |
| **Profissional de arquitetura** | Precisa de conhecimento muito mais profundo para decidir e orientar. Trade-offs, limites, evolução e risco exigem critério: sem profundidade, a orientação vira opinião; com ela, vira responsabilidade. |
